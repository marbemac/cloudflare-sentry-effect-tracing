...
export class MyDurableObject extends DurableObject<Env> {
  ...
  async runEffectWithError(props: WithTrace<ExampleProps>) {
    return continueTraceableRPC('durable-object-runEffectWithError', this.#runEffectWithError, this.ctx.waitUntil.bind(this.ctx), props);
  }
  ...
  async runForkedEffect(props: WithTrace<ExampleProps>) {
    return continueTraceableRPC('durable-object-runForkedEffect', this.#runForkedEffect, this.ctx.waitUntil.bind(this.ctx), props);
  }

  #runForkedEffect = async (_: ExampleProps) => {
    const res = await runForkedEffect();

    this.ctx.waitUntil(runtime.runPromise(res.backgroundFiber.pipe(Fiber.join, Effect.andThen(() => Effect.gen(function* () {
